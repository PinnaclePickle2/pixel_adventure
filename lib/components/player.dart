...
class Player extends SpriteAnimationGroupComponent
  ...
  //инвентарь
  late final InventoryManager inventoryManager = InventoryManager();
  ...
  final ValueNotifier<List<double>> notifiers =
      ValueNotifier([1.0, 1.0]); // 100% здоровья и 100% Stamina
  ...
  void _playerHit(double damageAmount) async {
    //звук
    if (game.playSounds) {
      //FlameAudio.play('collect_fruit.wav', volume: game.soundVolume);
    }
    ...
    //уменьшение здоровья
    notifiers.value = [
      (notifiers.value[0] - damageAmount).clamp(0.0, 1.0),
      notifiers.value[1]
    ];

    if (notifiers.value[0] == 0) gameOver();

    //удаление предметов из инвентаря и возврат списка удалённых предметов
    List<String>? dropItemsList = inventoryManager.deleteRandomItems();
    ...
    if (dropItemsList != null) {
      ...
      for (int i = 0; i < dropItemsList.length; i++) {
        ...
        final level = gameRef.children.whereType<Level>().first;
  ...
  void _playerSprint(double dt) {
    //уменьшение стамины
    if (notifiers.value[1] > 0.02) {
      moveSpeed = moveSpeedSprint;
      canSprint = true;
      notifiers.value = [
        notifiers.value[0],
        (notifiers.value[1] - ((notifiers.value[1] - 0.01) * dt))
            .clamp(0.0, 1.0)
      ];
    } else {
  ...
  void _playerUpdateSprintBar(double dt) {
    ...
    if (notifiers.value[1] > 0.1 && canSprint == false) {
      canSprint = true;
    }
  }
}
